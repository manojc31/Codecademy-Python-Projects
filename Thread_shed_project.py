...
for daily in daily_transactions:
    daily_transaction_split.append(daily.split('+'))
...
for transaction in daily_transaction_split:
    ...
    for data_point in transaction:
        transaction_list.append(data_point.replace('\n', '').strip(' '))
...
for sales_data in sales:
    total_sales += float(sales_data.strip('$'))
...
def color_count(thread):
    color_total = 0
    for thread_color in thread_sold_split:
        if thread == thread_color:
            color_total += 1
    return color_total
...
for color in colors:
    "thread shed sold {0} threads {1} thread today.".format(color_count(colors), colors)
# print(total_sales)
